...
import { createTagGroupSchema, createTagSchema, editTagSchema, updateTagGroupSchema } from "./schema";
...
export async function editTagAction(_prev: unknown, formData: FormData): Promise<SubmissionResult<string[]>> {
  const submission = parseWithZod(formData, { schema: editTagSchema });
  if (submission.status !== "success") {
    return submission.reply();
  }

  try {
    await db.update(tagsTable).set(submission.value).where(eq(tagsTable.id, submission.value.id));
    return submission.reply();
  } catch (e) {
    if (e instanceof DatabaseError) {
      if (e.constraint === "tags_group_id_name_unique") {
