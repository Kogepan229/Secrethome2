...
function CloseButton({ closeText, onClose }: { closeText: string; onClose: string | MouseEventHandler<HTMLButtonElement> }) {
...
export function MessageModal({
  ...
  return (
    <dialog ref={dialogRef} className="w-150 h-75 m-auto border-none rounded-sm backdrop:bg-black/60">
      ...
      <CloseButton closeText={closeText} onClose={onClose} />
    </dialog>
  );
}
